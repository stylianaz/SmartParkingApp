...
class Geocoder
{
    ...
    public function getDistanceMatrix($arrayofCoordinatesOrigins, $arrayofCoordinatesDestinations) : array{
        resolve(MatrixRequester::class)->acceptOriginsAndDestinations($arrayofCoordinatesOrigins, $arrayofCoordinatesDestinations);
        return resolve(MatrixRequester::class)->getDistanceMatrix();
    }

    public function getParkingsAroundALatLong($lat, $long, $radius = 1, $user_id = 1010, $role){
        $this->getParkingsAroundALatLongFromOpenDataApi($lat, $long, $radius, $user_id);
        $nearbyCarparks = StoredQueries::getNearbyCarparks($lat, $long, $radius);
        $nearbyCarparks = Utilities::convertCollectionToArrayOfArrays($nearbyCarparks);

        $carparkIds = array_keys($nearbyCarparks);
        if (!$this->isDatabaseResultValid($carparkIds)){
        ...
        if (!$this->isDatabaseResultValid($carparkIds)){
            return -1;
            //throw exception "could not get parking nearby"
        }

        $checkMinimum = $this->getMin($nearbyCarparks, 'Distance');
        ...
        $filtered = StoredQueries::getCarparksFilteredByRole($carparkIds, $role, $user_id);
        $filtered = Utilities::convertCollectionToArrayOfArrays($filtered);
        $finalIDs = array_keys($filtered);

        $allOtherDetails = StoredQueries::getCarparksDetailsFromIds($finalIDs);
        $allOtherDetails = Utilities::convertCollectionToArrayOfArrays($allOtherDetails);

        $result = Utilities::mergeArrays(array($filtered, $allOtherDetails, $nearbyCarparks));

        foreach($result as $key => $row){
            $result[$key]['url'] = $this->parkingUrlBaseonType($row);
            $result[$key]['placesCosts'] = Places::find($key)->placesCosts;
        }

        return $result;
    }

    private function isDatabaseResultValid($result){
        return isset($result) && is_array($result) && count($result) > 0;
    }
